...
#   decoder(self.tgt_embed(tgt), memory, src_mask, tgt_mask)
#       
...
def attention(query, key, value, mask=None, dropout=None):
    ...
    #query.size()=torch.Size([30, 8, 10, 64])
    #key.transpose(-2, -1).size()=torch.Size([30, 8, 64, 10])
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        #这里的multi-head 是将h个特征提取线性矩阵拼接在一起，即
...
class Batch:
    ...
    def __init__(self, src, trg=None, pad=0):
        ...
        if trg is not None:
            ...
            # 10×10 变为 9×9
            # for now see 
            # this is just a trick to produce different tgt and src data
            # training set is different from validate set?????
...
def run_epoch(data_iter, model, loss_compute):
    ...
    total_loss = 0
    tokens = 0
    #data_iter: yield Batch(src, tgt, 0)
    # 生成batch这个类
...
#SimpleLossCompute(model.generator, criterion, model_opt))   \
#criterion = LabelSmoothing(size=V, padding_idx=0, smoothing=0.0)1     
